...
* Argument `spec` has the form `{posArgSpecs, optArgSpecs, helpHeader}` or just `{posArgSpecs, optArgSpecs}`:
    * `posArgSpecs` is a list defining the specification for positional arguments. It has the form `{argName -> argSpec, ...}`, where `argName` is a string. Default values can be specified by using `{argName, default} -> argSpec` instead. Positional arguments without default cannot appear after positional arguments with defaults.
